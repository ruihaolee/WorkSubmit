<?php
    ...
    $savePath = realpath($path . '../../../../newImgs/member/') . DIRECTORY_SEPARATOR;
    $saveURL  = '../lastpack/newImgs/member/';
    //$savePath = realpath($path .'/newImgs/member/') . DIRECTORY_SEPARATOR;
   // $saveURL = 'newImgs/member/'; 
    ...
    {        
        ...
        $imageUploader->config(array(
            'maxSize' => 1024000,        // 允许上传的最大文件大小，以KB为单位，默认值为1024
            'cover'   => true         // 是否覆盖同名文件，默认为true
        ));
        //$imageUploader->saveName = date('ymdhis');
